...
* **vscode 확장 프로그램**(서버 실행하거나 프로젝트의 코드 내용을 살펴볼 때 권장함)
    ...
    - **Ruby on Rails(Hridoy)**: Rails의 각종 기능을 보조 해주는 확장 프로그램입니다.
    - **Ruby Solargrapt(Castwide)**: 루비의 클래스, 메서드, 문법 등을 설명해주고 자동 완성 기능 등 각종 기능을 제공해줍니다.
    - **SQLite Viewer(Florian Klampfer)**: vscode 내부에서 데이터를 살펴볼 때 필요한 확장 프로그램입니다.
    - **vscode-icons(VSCode Icons Team)**: vscode에서 열린 각종 폴더나 파일들의 아이콘을 기능에 맞는 이미지로 만들어줍니다.
    - **HTML End Tag Labels(Ante Primorac):** 코드가 길어질수록 태그 별로 구분하기 힘들어지는 문제를 이 확장 프로그램을 설치하면 각 태그 끝에 해당 태그의 첫번째 부분을 표시해주어 가독성을 높여줍니다.
<br>
