class FacilityReservationsController < ApplicationController
  ...
  def new
    @reservation = @facility.facility_reservations.build
  end

  def create
    @reservation = @facility.facility_reservations.build(reservation_params)
    @reservation.user = current_user
    @reservation.status = :pending # 초기 시설 대관 신청 상태
  ...
  def index # 사용자의 대관 신청 내역
    @reservations = current_user ? current_user.facility_reservations.includes(:facility).order(created_at: :desc) : []
  end

  def show
    @reservation = current_user.facility_reservations.includes(:facility).find(params[:id])
  end
